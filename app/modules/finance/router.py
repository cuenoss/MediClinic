...
async def expense_stats(db: AsyncSession = Depends(get_db)):
    """
    Expense totals and profit placeholders.
    Appointment rows have no payment_amount yet, so revenue from appointments is 0.
    """
    ...
    return ExpenseListResponse(
        ...
        todays_profit=float(todays_revenue - (todays_expenses or 0)),
        total_profit=float(total_revenue - (total_expenses or 0)),
    )
...
async def monthly_overview(db: AsyncSession = Depends(get_db)) -> Dict[str, float]:
    ...
    revenue = 0.0
    return {"revenue": float(revenue), "expenses": float(expenses or 0)}
